...
Controllers MUST be capable to discover receiver monitor objects (objects which implement [NcReceiverMonitor](https://specs.amwa.tv/nmos-control-feature-sets/branches/publish-status-reporting/monitoring/#ncreceivermonitor) directly or derive a [vendor specific variant from NcReceiverMonitor](https://specs.amwa.tv/ms-05-02/latest/docs/Introduction.html)) inside a device model. All blocks inside an MS-05-02 device allow [searching for members by their class id](https://specs.amwa.tv/ms-05-02/latest/docs/Blocks.html#search-methods).

Controllers MUST be capable to find the associated IS-04/IS-05 receiver identity for each receiver monitor by using the [touchpoints](#touchpoints-and-is-04-receivers).

Controllers MUST be capable to get the current state of the overallStatus property.

Controllers SHOULD be capable to get the current state of the following status properties:
...
Controllers MUST be capable to track changes to the overallStatus property by using subscriptions and notifications:

Controllers SHOULD be capable to track changes to the following status properties by using subscriptions and notifications:
...
Controllers MAY be capable of getting the current synchronizationSourceId and track changes made to it via notifications.

Controllers MAY be capable of getting the current state of a status message and track changes made to it via notifications.

The values of status message properties are meant to be used as is and MUST not be interpreted by controllers.
...
Controllers MAY be capable of getting the lost packets counters from a devices which offers them.

Controllers MAY be capable of getting the late packets counters from a devices which offers them.
...
Controllers MAY be capable to invoke the ResetCounters method.

Controllers MAY be capable to set the autoResetCounters property.

Controllers MAY be able to get and track changes to the values of any status transition counter properties.
